class JobOrdersController < ApplicationController
  ...
  def index
    ...
    if params[:search]
        @job_orders = JobOrder.search(params[:search]).order("created_at DESC")
      else
        @job_orders = JobOrder.all.order('created_at DESC')
      end
  end
  ...
  private
    ...
    def job_order_params
      params.require(:job_order).permit(:clientId, :teamId, :netAmount, :grossAmount, :description, :startTime, :stopTime, :materials)
    end
end
